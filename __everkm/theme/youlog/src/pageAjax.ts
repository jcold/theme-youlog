...
async function loadPageContent(url: string): Promise<boolean> {
  try {
    ...
    // 获取所需的元素
    const newArticleMain = doc.getElementById('article-main')
    const newHeader = doc.querySelector('h1[data-article-title]')
    const newTitle = doc.querySelector('title')
    const newPageIndicator = doc.getElementById('page-indicator')
    const newBreadcrumb = doc.getElementById('breadcrumb')
    const newArticleTitle = doc.getElementById('article-title')
    // 检查是否找到了必要的元素
    if (!newArticleMain || !newHeader) {
      return false // 返回失败状态
    }

    // 更新页面内容
    const currentArticleMain = document.getElementById('article-main')
    if (currentArticleMain) {
      currentArticleMain.innerHTML = newArticleMain.innerHTML
    }

    // 更新标题
    const currentHeader = document.querySelector('h1[data-article-title]')
    if (currentHeader) {
      currentHeader.textContent = newHeader.textContent
    }

    // 更新文章标题
    const currentArticleTitle = document.getElementById('article-title')
    if (currentArticleTitle) {
      currentArticleTitle.textContent = newArticleTitle?.textContent || ''
    }

    // 更新页面指示器
    const currentPageIndicator = document.getElementById('page-indicator')
    if (currentPageIndicator) {
      currentPageIndicator.innerHTML = newPageIndicator?.innerHTML || ''
    }
    ...
    // 更新面包屑
    const currentBreadcrumb = document.getElementById('breadcrumb')
